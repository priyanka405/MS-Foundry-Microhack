...
| **Tool integration** | All three tools reachable via the shipped channel. A scheduled Azure Function is added *only* for the batch renewal-reminder job (a non-conversational workload the agent should not own). |
...
3. *"What is the current status of the Contoso NDA?"*
