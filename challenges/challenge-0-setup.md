...
| **Agent capability** | Foundation only &mdash; no agent behavior yet. This challenge lays the runtime the Contract Lifecycle Management Agent will live on. |
| **Tool integration** | None yet. Prepares the substrate for every tool that follows. |
| **Azure services used** | Azure AI Foundry, Azure AI Foundry Models (gpt-4o / gpt-4o-mini), Azure AI Search, Azure Blob Storage, Application Insights. |
| **Expected outcome** | A working Foundry project with a deployed model, an indexed corpus, and end-to-end tracing enabled &mdash; ready for Challenge 1. |
