...
> **Design note.** Approval routing runs on Power Automate (native Microsoft 365 approval primitives, not a bespoke Logic App). Contract status lives in Dataverse (or Azure SQL) &mdash; a data connector, not a hand-rolled Azure Function &mdash; so the agent reads and writes structured lifecycle state deterministically. Clause analysis is a language task the Foundry model performs directly; there is no external microservice.
...
### Connected service
**Power Automate** &mdash; approval flows are already the standard in Microsoft 365 (SharePoint approvals, Teams approvals, Outlook approvals). Power Automate is the native Foundry Agent Actions integration for those primitives, so we use it instead of hand-rolling a Logic App.
